...
#if defined(NDEBUG)
# error "Adamantlegioncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb402cd8d4c7208a40504f12d3c09f6440216620378fa9ab81d2537eddc2a7f9a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Adamantlegioncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Adamantlegioncoin Signed Message:\n";
...
{
    // Adamantlegioncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Adamantlegioncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1* 30; // Adamantlegioncoin: 30 seconds
static const int64 nTargetSpacing = 1 * 10; // Adamantlegioncoin: 10 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Adamantlegioncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Adamantlegioncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb402cd8d4c7208a40504f12d3c09f6440216620378fa9ab81d2537eddc2a7f9a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Adamant Legion Coin ALC AdamantLegion.com ts3.adamantlegion.com";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc3a7e4d3eb4b20f4d4704fa08dce601aaf0f270216fe1b51850b4acf21b179c45070ac7b03a1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1485906124;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1414516;
        ...
        assert(block.hashMerkleRoot == uint256("0xe5feadde4d44bdbee04d0796804aebd4f9fb258dbc34bf5ba07f806cfe64f423"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Adamantlegioncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AdamantlegioncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AdamantlegioncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AdamantlegioncoinMiner(CWallet *pwallet)
{
    printf("AdamantlegioncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("adamantlegioncoin-miner");
    ...
    try { loop {
        ...
        printf("Running AdamantlegioncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AdamantlegioncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AdamantlegioncoinMiner, pwallet));
}
